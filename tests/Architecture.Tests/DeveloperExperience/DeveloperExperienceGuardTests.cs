...
public sealed partial class DeveloperExperienceGuardTests
{
    ...
    public void Dotnet_ef_tool_manifest_matches_entity_framework_design_package()
    {
        ...
        string addMigrationScript = File.ReadAllText(Path.Combine(
            repositoryRoot,
            "gma",
            "framework",
            "eng",
            "add-migration.ps1"));
        string checkMigrationsScript = File.ReadAllText(Path.Combine(
            repositoryRoot,
            "gma",
            "framework",
            "eng",
    ...
    {
        string repositoryRoot = FindRepositoryRoot();
        string wrapper = File.ReadAllText(Path.Combine(repositoryRoot, "eng", "check-submodule-dev-heads.ps1"));
        ...
        string commonScript = File.ReadAllText(Path.Combine(
            repositoryRoot,
            "gma",
            "framework",
            "eng",
            "composition-common.ps1"));
        string scriptSurface = script + commonScript;
        ...
        [
            ...
            "diff', '--cached', '--quiet'",
            "status', '--porcelain'",
            "All selected submodule pointers match their configured remote branches."
        ];
        ...
        string[] offenders = submodulePaths
            ...
            .Concat(!wrapper.Contains("gma/framework/eng/check-submodule-heads.ps1", StringComparison.OrdinalIgnoreCase)
                ? ["eng/check-submodule-dev-heads.ps1 should delegate to the Framework-owned guard."]
                : [])
            .Concat(requiredScriptTokens
                .Where(token => !scriptSurface.Contains(token, StringComparison.Ordinal))
    ...
    {
        string repositoryRoot = FindRepositoryRoot();
        string wrapper = File.ReadAllText(Path.Combine(repositoryRoot, "eng", "check-source-packages.ps1"));
        string checker = File.ReadAllText(Path.Combine(
            repositoryRoot,
            "gma",
            "framework",
            "eng",
        ...
        string[] requiredCheckerTokens =
        [
            "Get-GmaCompositionSubmodules",
            "No matching source packages were found in .gitmodules.",
            "Get-ChildItem -LiteralPath $packageRoot -Filter '*.slnx' -File"
        ];
        string[] offenders = focusedSolutions
            .Select(solution => validationScript.Contains(solution, StringComparison.OrdinalIgnoreCase)
                ? string.Empty
                : $"eng/gma-validate.ps1 missing {solution}")
            .Where(offender => offender.Length > 0)
            .Concat(requiredCheckerTokens
                .Where(token => !checker.Contains(token, StringComparison.Ordinal))
    ...
    public void Verify_script_checks_provider_migration_drift()
    {
        string repositoryRoot = FindRepositoryRoot();
        string verifyScript = File.ReadAllText(Path.Combine(repositoryRoot, "eng", "verify.ps1"));
        string migrationCheckWrapper = File.ReadAllText(Path.Combine(repositoryRoot, "eng", "check-migrations.ps1"));
        string migrationCheckScript = File.ReadAllText(Path.Combine(
            repositoryRoot,
            "gma",
            "framework",
            "eng",
        ...
        string[] offenders = requiredVerifyTokens
            ...
            .Concat(!migrationCheckWrapper.Contains("gma/framework/eng/check-migrations.ps1", StringComparison.OrdinalIgnoreCase)
                ? ["eng/check-migrations.ps1 should delegate to the Framework-owned checker."]
                : [])
    ...
    public void Add_migration_script_preserves_single_dbcontext_discovery_as_array()
    {
        ...
        Assert.Contains("$contextNames = @(Get-ChildItem", addMigrationScript, StringComparison.Ordinal);
        Assert.Contains("} | Sort-Object -Unique)", addMigrationScript, StringComparison.Ordinal);
        ...
        [
            ...
            new(
                ...
                [
                    ...
                    @"..\Modules\Organizations\Gma.Modules.Organizations.AdminApi\Gma.Modules.Organizations.AdminApi.csproj",
                    @"..\Modules\Organizations\Gma.Modules.Organizations.Persistence.PostgreSqlMigrations\Gma.Modules.Organizations.Persistence.PostgreSqlMigrations.csproj",
                    @"..\Modules\Organizations\Gma.Modules.Organizations.Persistence.SqlServerMigrations\Gma.Modules.Organizations.Persistence.SqlServerMigrations.csproj",
            ...
            new(
                ...
                [
                    ...
                    @"..\Modules\Auth\Gma.Modules.Auth.Persistence.PostgreSqlMigrations\Gma.Modules.Auth.Persistence.PostgreSqlMigrations.csproj",
                    @"..\Modules\Auth\Gma.Modules.Auth.Persistence.SqlServerMigrations\Gma.Modules.Auth.Persistence.SqlServerMigrations.csproj",
                    @"..\Modules\Organizations\Gma.Modules.Organizations.AdminCli\Gma.Modules.Organizations.AdminCli.csproj",
                    @"..\Modules\Organizations\Gma.Modules.Organizations.Persistence.PostgreSqlMigrations\Gma.Modules.Organizations.Persistence.PostgreSqlMigrations.csproj",
                    @"..\Modules\Organizations\Gma.Modules.Organizations.Persistence.SqlServerMigrations\Gma.Modules.Organizations.Persistence.SqlServerMigrations.csproj",
            ...
            new(
                ...
                [
                    ...
                    @"..\Extensions\Gma.Extensions.Organizations.AccessControl\Gma.Extensions.Organizations.AccessControl.csproj",
            ...
            new(
                ...
                [
                    ...
                    @"..\Modules\Notifications\Gma.Modules.Notifications.Persistence.PostgreSqlMigrations\Gma.Modules.Notifications.Persistence.PostgreSqlMigrations.csproj",
                    @"..\Modules\Notifications\Gma.Modules.Notifications.Persistence.SqlServerMigrations\Gma.Modules.Notifications.Persistence.SqlServerMigrations.csproj",
                    @"..\Modules\Organizations\Gma.Modules.Organizations.Application\Gma.Modules.Organizations.Application.csproj",
                    @"..\Modules\Organizations\Gma.Modules.Organizations.Contracts\Gma.Modules.Organizations.Contracts.csproj",
                    @"..\Modules\Organizations\Gma.Modules.Organizations.Persistence\Gma.Modules.Organizations.Persistence.csproj",
                    @"..\Modules\Organizations\Gma.Modules.Organizations.Persistence.PostgreSqlMigrations\Gma.Modules.Organizations.Persistence.PostgreSqlMigrations.csproj",
                    @"..\Modules\Organizations\Gma.Modules.Organizations.Persistence.SqlServerMigrations\Gma.Modules.Organizations.Persistence.SqlServerMigrations.csproj",
